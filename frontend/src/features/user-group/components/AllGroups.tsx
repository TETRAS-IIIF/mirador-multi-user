...
export const AllGroups = ({
  ...
  return (
    <>
      <SidePanelMedia open={openSidePanel && !!openModalGroupId} setOpen={handleSetOpenSidePanel}
                      display={!!openModalGroupId} fetchMediaForUser={fetchMediaForUser} medias={medias} user={user}
                      userPersonalGroup={userPersonalGroup!}>
        <Grid item container flexDirection="column">
          <Grid item container justifyContent="flex-end" direction="row" spacing={2} alignItems="center"
            ...
            <Grid item>
              <SearchBar handleFiltered={handleFiltered} label={t('filterGroups')} fetchFunction={handleLookingForGroup}
                         getOptionLabel={getOptionLabel} setSelectedData={setSelectedUserGroup} />
            </Grid>
          </Grid>
            ...
            {groups && groupFiltered && groupFiltered.length < 1 && !selectedUserGroup && currentPageData.map((group) => (
              <Grid item key={group.id}>
                <MMUCard
                  ...
                  EditorButton={<ModalButton tooltipButton={t('editGroupTooltip')} disabled={false}
                                             icon={<ModeEditIcon />}
                                             onClickFunction={() => HandleOpenModal(group.id)} />}
                  ReaderButton={<ModalButton disabled={true} tooltipButton={t('openGroupTooltip')}
                                             icon={<ModeEditIcon />}
            ...
            {selectedUserGroup && (
              <Grid item>
                <MMUCard
                  ...
                  EditorButton={<ModalButton tooltipButton={t('editGroupTooltip')} disabled={false}
                                             icon={<ModeEditIcon />}
                                             onClickFunction={() => HandleOpenModal(selectedUserGroup.id)} />}
                  ReaderButton={<ModalButton tooltipButton={t('openGroupTooltip')} disabled={true}
                                             icon={<ModeEditIcon />}
                                             onClickFunction={() => console.log('you\'re not allowed to do this')} />}
          ...
          <DrawerCreateGroup handleCreatGroup={handleCreateGroup} modalCreateGroup={modalGroupCreationIsOpen}
